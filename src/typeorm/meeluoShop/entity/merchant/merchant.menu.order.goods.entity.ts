...
import { MerchantMenuOrderEntity } from './merchant.menu.order.entity';
import { MerchantUserEntity } from './merchant.user.entity';
import { MerchantCouponEntity } from './merchant.coupon.entity';

export enum MerchantMenuOrderGoodsIsCommentEnum {
  TRUE = 1,
  FALSE = 0,
}

@Scope<MerchantMenuOrderGoodsEntity>([{ column: 'is_delete', value: 0 }])
@Entity('merchant_menu_order_goods', { database: MEELUO_SHOP_DATABASE })
export class MerchantMenuOrderGoodsEntity extends MerchantBaseEntity {
  @Column('bigint', {
    unsigned: true,
    nullable: true,
  ...
  @Column('tinyint', {
    comment: '是否已评论，1是 0否',
    unsigned: true,
    default: MerchantMenuOrderGoodsIsCommentEnum.FALSE,
  })
  isComment: MerchantMenuOrderGoodsIsCommentEnum;
  ...
  @ManyToOne(() => MerchantMenuOrderEntity)

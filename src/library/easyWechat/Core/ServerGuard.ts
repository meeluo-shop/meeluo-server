...
export default class ServerGuard {
  static SUCCESS_EMPTY_RESPONSE = 'success';
   ...
   * @param condition EasyWechat.Messages.Message.xxx，用于处理特定消息类型，默认：* 表示全部
   */
  push(handler: Function, condition = '*'): void {
  ...
  protected async notify(event: string, payload: any): Promise<any> {
    ...
    for (const condition in this.handlers) {
      const handlers = this.handlers[condition];
      if (
        condition == '*' ||
        condition == event ||
        Number(Number(condition) & Number(event)) == Number(event)
      ) {
        let isBreak = false;
        for (let i = 0; i < handlers.length; i++) {
          const handler = handlers[i];

          const res = await this._callHandler(handler, payload);
  ...
  protected async validate(): Promise<ServerGuard> {
    ...
    const signature = await this.app['request'].get('signature');
    const timestamp = await this.app['request'].get('timestamp');
    const nonce = await this.app['request'].get('nonce');
  ...
  protected async buildResponse(
    ...
    if (!(message instanceof Message)) {
      throw new Error(
        'The message object should be instance of EasyWechat.Messages.*',
      );
    }
  ...
  protected async buildReply(
    ...
    const res = message.transformToXml(prepends);
  ...
  protected async isSafeMode(): Promise<boolean> {
    const signature = await this.app['request'].get('signature');
    const encrypt_type = await this.app['request'].get('encrypt_type');
  ...
  protected async handleRequest(): Promise<object> {
    const castedMessage = await this.getMessage();

    const response = await this.notify(
      ServerGuard.MESSAGE_TYPE_MAPPING[
        castedMessage['MsgType'] || castedMessage['msg_type'] || 'text'
      ],
      castedMessage,
    );

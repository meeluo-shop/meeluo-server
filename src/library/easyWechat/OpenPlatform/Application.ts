...
export default class OpenPlatform extends BaseApplication {
  ...
  async getPreAuthorizationUrl(
    ...
    if (isString(optional)) {
      ...
      optional['pre_auth_code'] = (await this.createPreAuthorizationCode())[
        'pre_auth_code'
      ];
    }
   ...
   * @param optional 其它参数
   */
  async getMobilePreAuthorizationUrl(
    callbackUrl: string,
    optional = {},
  ): Promise<string> {
    if (isString(optional)) {
  ...
  protected getAuthorizerConfig(
    appId: string,
    refreshToken: string = null,
  ): object {
    return Merge({}, this.config, {
      component_app_id: this.config['appid'],
      appid: appId,
  ...
  officialAccount(
    appId: string,
    refreshToken: string = null,
    accessToken: AuthorizerAccessToken = null,
  ) {
    const that = this;
    const services = Merge({}, this.getReplaceServices(accessToken), {

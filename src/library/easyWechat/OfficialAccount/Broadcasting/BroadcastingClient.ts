...
export default class BroadcastingClient extends BaseClient {
  ...
  previewMessage(
    message: Message,
    reception: string,
    method: string = this.PREVIEW_BY_OPENID,
  ): Promise<any> {
    const msg = new MessageBuilder()
      .message(message)
      .buildForPreview(method, reception);

...
export default class Client extends BaseClient {
   ...
   */
  create(cardType = '', attributes: object): Promise<any> {
  ...
  async getQrCode(ticket: string): Promise<object> {
    const baseUrl = 'https://mp.weixin.qq.com/cgi-bin/showqrcode';
    const params = {
      ticket,
    };
   ...
   * @param cardId 卡券id
   */
  getUserCards(openid: string, cardId = ''): Promise<any> {
   ...
   */
  setPayCell(cardId: string, isOpen = true): Promise<any> {
  ...
  protected updateStock(
    ...
    const key =
      'increase' === action ? 'increase_stock_value' : 'reduce_stock_value';

...
import { AdminIsMenuCategoryEnum } from '@typeorm/meeluoShop';
import { ApiProperty } from '@shared/swagger';
import { IsEnum } from '@core/decorator';

export class AdminUpdateMenuDTO {
  @ApiProperty({
    description: '导航名称',
  })
  ...
  @IsOptional()
  @MaxLength(200, { message: '重定向长度不能超过200' })
  redirect: string;
...
export class AdminCreateMenuDTO extends AdminUpdateMenuDTO {
  @ApiProperty({
    enum: AdminIsMenuCategoryEnum,
    description: '是否是分类',
  })
  @Type(() => Number)
  @IsEnum(AdminIsMenuCategoryEnum, { message: 'isCategory参数格式错误' })
  isCategory: AdminIsMenuCategoryEnum = AdminIsMenuCategoryEnum.FALSE;
}

export class AdminMenuIdDTO {
  @ApiProperty({
    description: '导航id',
  })
  @Type(() => Number)
...
export class AdminMenuIdListDTO {
  @ApiProperty({
    type: [Number],
    description: '导航id列表',
  })
